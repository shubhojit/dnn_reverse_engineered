...
from keras.layers.embd import Embedding
from keras.models import Model
from keras.layers.merge import concatenate

word2vec = gensim.models.Word2Vec.load("word2vec.gensim")

# In order to represent unseen words & null token; added an additional row of zeros to the embedding matrix.
embd = np.zeros((word2vec.syn0.shape[0] + 1, word2vec.syn0.shape[1]), dtype = "float32")
embd[:word2vec.syn0.shape[0]] = word2vec.syn0

maximum_tokens = word2vec.syn0.shape[0]
embedding_dimension = word2vec.syn0.shape[1]
...
embedder = Embedding(maximum_tokens + 1, embedding_dimension, weights = [embd], trainable = False)
document_embedding = embedder(document)
left_embedding = embedder(left_context)
right_embedding = embedder(right_context)


forward = LSTM(hidden_dimension_1, return_sequences = True)(left_embedding) # Using LSTM RNNs instead of vanilla RNNs. As detailed in equation (1) of the paper.
backward = LSTM(hidden_dimension_1, return_sequences = True, go_backwards = True)(right_embedding) # equation (2) of the paper.
together = concatenate([forward, document_embedding, backward], axis = 2) # Equation (3).

semantic = TimeDistributed(Dense(hidden_dimension_2, activation = "tanh"))(together) # Equation (4) of the paper using tanh activation .

pool_rnn = Lambda(lambda x: backend.max(x, axis = 1), output_shape = (hidden_dimension_2, ))(semantic) # See equation (5).

output = Dense(NUM_CLASSES, input_dimension = hidden_dimension_2, activation = "softmax")(pool_rnn) # Equations (6) &(7) of the paper.
