...
def loading_data(f):
  batch = file_unpickle(f)
  data = batch['data']
  labels = batch['labels']
  print "Loading the data %s: %d" % (f, len(data))
  return data, labels

def load_data(files, data_dir, label_count):
  data, labels = loading_data(data_dir + '/' + files[0])
  for f in files[1:]:
    data_n, labels_n = loading_data(data_dir + '/' + f)
...
def run():
  ...
  meta = file_unpickle(data_dir + '/batches.meta')
